...
export class AuthService {
  ...
  async login(email: string, password: string): Promise<Response<LoginResponseDto>> {
  ...
  async register(email: string, password: string, fullName: string, phoneNumber: string, role: number): Promise<Response<boolean>> {
    return await this.authGateway.register({ email, password, fullName, phoneNumber, role });
  }
